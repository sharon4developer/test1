...
This is a second, independent agent alongside `career-agent/` (CareerBot, Canada-wide IT job search, bound to the existing `@SharonCareerBot`). CareerBotSask has its own workspace, agent id (`careerbotsask`), Telegram bot ("CareerBotSask", created fresh in BotFather), application tracker, daily apply cap, and resume identity — the two tracks do not share state.
