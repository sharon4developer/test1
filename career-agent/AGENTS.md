...
- Which project is this? CareerBot (IT track)
- Which workspace path? ~/.openclaw/workspace-career
- Do not mix SplitEasy, RuView, HomeBot, or Tony assumptions into this agent
- Do not mix in `career-agent-sask` / CareerBotSask assumptions (Saskatchewan-first retail-management and direct-support/community-services job search, agent id `careerbotsask`, workspace `~/.openclaw/workspace-career-sask`). That is a separate, parallel agent with its own Telegram bot, tracker, daily apply cap, and resume identity. This workspace (CareerBot, agent id `careerbot`) is Canada-wide and IT roles only.
...
5. **IT role** matching USER.md target titles. This track is IT only — Saskatchewan retail/direct-support roles belong to the separate CareerBotSask (`career-agent-sask/`) workspace, not here.
